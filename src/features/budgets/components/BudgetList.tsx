...
export default function BudgetList({ monthIso }: BudgetListProps) {
  const { data: budgets = [], isLoading, isError, error } = useGetBudgetsQuery(monthIso);
  ...
  if (!budgets || budgets.length === 0) {
    return <div className="text-gray-500 text-center py-8">No budgets found for this month.</div>;
  }

  return (
    <div className="space-y-4">
      <h3 className="text-lg font-medium text-gray-900">Budgets for {monthIso}</h3>
          ...
          <li key={budget.id} className="py-4 flex justify-between items-center">
            <div className="flex flex-col">
              ...
              {budget.spent !== undefined && (
                <span className="text-sm text-gray-500">Spent: ₹{budget.spent.toFixed(2)}</span>
              )}
            </div>
